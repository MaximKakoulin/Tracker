...
//    private var trackerName: String?
    ...
    var colorIndex: Int? {
        return colorData.firstIndex(where: { $0.hexString() == self.chosenColor?.hexString() })
       }
    ...
    private func selectInitialEmojiAndColor() {
        // Выберите элементы в коллекциях, используя значения chosenEmoji и chosenColor
        if let chosenEmoji = chosenEmoji,
           let index = emojiData.firstIndex(of: chosenEmoji) {
        ...
        if let chosenColor = chosenColor,
           let index = colorData.firstIndex(where: { $0.hexString() == chosenColor.hexString() }) {
            ...
            print("Chosen color is nil or not found in colorData.")
        }
    }
    ...
    private func getSelectedDaysString(from chosenDays: [Int]) -> String {
...
extension EditingTrackerViewController: UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if indexPath.row == 0 {
            ...
            let scheduleVC = ScheduleViewController(chosenDays: chosenDays)
...
//        ВОТ ТУТ НУЖНО СДелАТЬ пРОВерку
        if indexPath.row == 0 {
            ...
            let daysDescription = getSelectedDaysString(from: chosenDays)
            if chosenDays.count == 7 {
...
extension EditingTrackerViewController: UITextFieldDelegate {
    func textFieldDidBeginEditing(_ textField: UITextField) {
        if category == nil || chosenDays.isEmpty {
            showReminderAlert()
